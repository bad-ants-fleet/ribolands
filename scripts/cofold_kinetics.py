...
def all_forward_hyper_rates(RG, xxx_todo_changeme):
    """ forward rate: A + B => AB """
    (nodeA, nodeB) = xxx_todo_changeme
    forward = []
...
def all_backward_hyper_rates(RG, xxx_todo_changeme1):
    """ backward rate: AB => A + B """
    (nodeAB,) = xxx_todo_changeme1
    backward = []
...
def interkin_equilibrium(species, args):
  ...
  '''
    ...
    # -kT*math.log(sum([math.e**(-x/kT) for x in diffG]))
    D_ApB = F_ApB + sum(diffG)
    print "F", F_ApB
    print "T", T_ApB
    print "D", D_ApB
...
def add_edges(RG, bfile, rfile, mode, spe, seq, snodes, hcount):
    ...
    """
    RM = []
    # map(lambda(x): (spe, x[1]), ril.parse_barfile(bfile, seq=seq))
    lmins = []
    for line in ril.parse_barfile(bfile, seq=seq):
            ...
            for row in pDF:
                ...
                if len(cplR) == len(cplC):
                    ...
                    if len(cplR) == 1:
                        ...
                        hcount[0] += 1
                        hnode = ('HYPER', str(hcount[0]))
                        RG.add_weighted_edges_from(
                            [(row, hnode, pDF[col][row])])
                        RG.add_weighted_edges_from(
                            [(hnode, col, pDF[col][row])])
                        RG.node[hnode]['rate'] = pDF[col][row]
                        #RG.add_weighted_edges_from([(row, col, pDF[col][row])])
                        snodes[row] = 1
                        snodes[col] = 1
                        ...
                        if cplR[0] != cplC[0]:
                            ...
                            if edict:
                                ...
                                hnode = ('HYPER', str(hcount[0]))
                                RG.add_weighted_edges_from(
                                    [(cplR[0], hnode, pDF[col][row])])
                                RG.add_weighted_edges_from(
                                    [(hnode, cplC[0], pDF[col][row])])
                                RG.node[hnode]['rate'] = pDF[col][row]
                                #RG.add_weighted_edges_from([(cplR[0], cplC[0], pDF[col][row])])
                            snodes[cplR[0]] = 1
                            snodes[cplC[0]] = 1
                        if cplR[1] != cplC[1]:
                            # print "adding", cplR[1], cplC[1], pDF[col][row],
                            # row, col
                            ...
                            if edict:
                                ...
                                hnode = ('HYPER', str(hcount[0]))
                                RG.add_weighted_edges_from(
                                    [(cplR[1], hnode, pDF[col][row])])
                                RG.add_weighted_edges_from(
                                    [(hnode, cplC[1], pDF[col][row])])
                                RG.node[hnode]['rate'] = pDF[col][row]
                    ...
                    for tup in cplR:
                        RG.add_weighted_edges_from(
                            [(tup, hnode, pDF[col][row])])
                    for tup in cplC:
                        RG.add_weighted_edges_from(
                            [(hnode, tup, pDF[col][row])])
                    RG.node[hnode]['rate'] = pDF[col][row]
    else:
...
def get_complexes(xxx_todo_changeme2):
  ...
  that is important for homodimers """
    for cocp in sorted(list(nx.connected_components(graph)),
        ...
        stru = ''
        comp = ''
        for c, (ch, ss) in sorted(
                list(cocp), key=lambda x: x[0], reverse=False):
            stru += ss + '&'
            comp += ch + '&'
        stru = stru[:-1]
        comp = comp[:-1]
...
def get_interkin_args():
    ...
    # TODO: need to think whether this is the best way to do it... hidden, for
    # now!
