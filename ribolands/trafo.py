...
from ribolands.pathfinder import get_fpath_cache, get_bp_change
...
class TrafoLandscape(PrimePathLandscape):
    def __init__(self, *kargs, **kwargs):
        ...
        #self._full_sequence = self.sequence
        #self._model_details = self.md
        #self._fold_compound = self.fc
        #self._RT = self.RT
        ...
        self.fpath = 2   # findpath_search_width
        #self.dG_max = 0  # set using t_slow
    ...
    def expand(self, extend = 1, mfree = 6):
        ...
        if len(self) == 0:
            ...
            nn, _ = self.connect_to_active(nodes = [mfess], direct = False)
            print('connected mfe', mfess[:len(seq)], 'new structs:', len(nn))

            # Save the set of active parent nodes for later.
            parents = self.active_nodes

            # They be previously inactive nodes or other parents ...
            ...
            for p, fn in fraying_nodes.items():
                ...
                n1, _ = self.connect_to_active(nodes = list(fn), direct = False)
                # Connect fraying neighbors with each other
                n2, _ = self.connect_nodes_n2(nodes = list(fn), direct = True)
                newn |= n1 | n2
        ...
        if not reg_flt.match(lastlines[0]):
            ...
            if reg_flt.match(lastlines[1]):
                ...
                for e, occu in enumerate(lastlines[1].split()[1:]):
                    ss = snodes[e]
                    #self.nodes[ss]['occupancy_change'] = float(occu) - self.nodes[ss]['occupancy']
                ...
                for e, occu in enumerate(lastlines[0].split()[1:]):
                    ...
                    #self.nodes[ss]['occupancy_change'] = float(occu) - sso
                    self.nodes[ss]['occupancy'] = float(occu)/tot_occ
                    #print('UpdOcc', ss, self.nodes[ss]['identity'], sso, self.nodes[ss]['occupancy'])

        return time, iterations

    def prune(self, pmin):

        def move_occupancy(node):
            # keep if it connects multiple 
            no = self.nodes[node]['occupancy']
            ni = self.nodes[node]['identity']

            # Find energy barriers to all active structures?
            nn, _ = self.connect_to_active(nodes = [node], direct = True)
            assert len(nn) == 0

            nbrs = [n for n in self.successors(node) if self.nodes[n]['active']]
            if len(nbrs) > 0:
                merge = sorted(nbrs, key = lambda x: self[node][x]['weight'], reverse = True)
                print(node, ni, no)
                #print(nbrs)
                #for mer in merge:
                #    print(mer, self.nodes[mer]['identity'], self[n][mer]['weight'])
                assert self.nodes[merge[0]]['active']
                self.nodes[merge[0]]['occupancy'] += self.nodes[node]['occupancy']
...
# ~~~~ 
class OldTrafoLandscape(nx.DiGraph):
    """ 
    ...
    def __init__(self, fullseq, vrna_md):
        super(OldTrafoLandscape, self).__init__()
    ...
    def graph_copy(self):
        ...
        """
        copy = OldTrafoLandscape(self.full_sequence, self._model_details)
